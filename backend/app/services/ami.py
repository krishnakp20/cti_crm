...
class AMIClient:
    ...
    async def _handle_packet(self, pkt: dict):
        ...
        if event == "QueueCallerJoin":
            ...
            await self._broadcast({
                "type": "agent_complete",
                "uniqueid": uid,
                "agent": pkt.get("AgentName", call.get("agent", "")),
                "caller": call.get("caller", ""),
                "queue": pkt.get("Queue", call.get("queue", "")),
                "talk_time": pkt.get("TalkTime", "0"),
                "hold_time": pkt.get("HoldTime", "0"),
            })
            await self._save_cdr_complete(uid, pkt)

        elif event == "Hangup":
            # Abandoned call (caller hung up in queue)
            if uid in self._queue_callers:
                del self._queue_callers[uid]
    ...
    async def _save_cdr_agent_connect(self, uid: str, pkt: dict):
        try:
            from app.models.cdr import CallRecord
            ...
            async with await self._db_session() as db:
                ...
                if not rec:
                    rec = CallRecord(asterisk_unique_id=uid, caller_number=pkt.get("CallerIDNum", ""))
                    db.add(rec)

                now = datetime.now(timezone.utc)
                agent_ext = pkt.get("AgentName", "").replace("PJSIP/", "").split("-")[0]
                rec.call_start_time = now
                rec.call_status = "answered"
                rec.agent_extension = agent_ext
                rec.queue_name = pkt.get("Queue", "")
                rec.department = _queue_to_dept(pkt.get("Queue", ""))

                if rec.queue_start_time:
                    rec.queue_duration = int((now - rec.queue_start_time.replace(tzinfo=timezone.utc)).total_seconds())
